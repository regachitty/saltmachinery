...
import "../assets/css/GlobeComponent.css";

const GlobeComponent = () => {
  const containerRef = useRef(null);
  const canvasRef = useRef(null);
  const svgMapRef = useRef(null);
  const svgCountryRef = useRef(null);
  ...
  useEffect(() => {
    ...
    const svgMapDomEl = svgMapRef.current;
    const svgCountryDomEl = svgCountryRef.current;

    if (!svgMapDomEl || !svgCountryDomEl) {
      console.error("SVG elements not found!");
      return;
    }

    const svgCountries = Array.from(svgMapDomEl.querySelectorAll("path"));
    const countryNameEl = document.querySelector(".globe-info span.globe-span");
    ...
    return () => {
      ...
      containerEl.removeEventListener("touchstart", (e) => {
        isTouchScreen = true;
      });
      containerEl.removeEventListener("mousemove", (e) => {
        updateMousePosition(e.clientX, e.clientY);
      });
      containerEl.removeEventListener("click", (e) => {
        updateMousePosition(e.clientX, e.clientY);
      });
    };
  }, []);
  ...
  const setMapTexture = (material, URI) => {
  ...
  const prepareHiResTextures = () => {
    const svgMapDomEl = svgMapRef.current;
    const svgCountries = Array.from(svgMapDomEl.querySelectorAll("path"));
    const countryNameEl = document.querySelector(".globe-info span.globe-span");

    if (!svgMapDomEl || !svgCountries.length) {
      console.error("SVG map or countries not found!");
      return;
    }
    ...
    gsap.set(svgMapDomEl, {
      attr: {
        viewBox: `0 ${offsetY * svgViewBox[1]} ${svgViewBox[0]} ${
          svgViewBox[1]
        }`,
    ...
    svgCountries.forEach((country) => {
      ...
      if (countryColors[countryName]) {
        country.setAttribute("fill", countryColors[countryName]);
      }
    });

    const svgData = new XMLSerializer().serializeToString(svgMapDomEl);
    staticMapUri =
      "data:image/svg+xml;charset=utf-8," + encodeURIComponent(svgData);
    setMapTexture(globeColorMesh.material, staticMapUri);

    // Prepare strokes (unchanged)
    gsap.set(svgMapDomEl, {
      attr: {
        fill: "none",
        stroke: params.strokeColor,
    ...
    const strokeSvgData = new XMLSerializer().serializeToString(svgMapDomEl);
    staticMapUri =
      "data:image/svg+xml;charset=utf-8," + encodeURIComponent(strokeSvgData);
    setMapTexture(globeStrokesMesh.material, staticMapUri);
  };

  const prepareLowResTextures = () => {
    const svgCountryDomEl = svgCountryRef.current;
    const svgMapDomEl = svgMapRef.current;
    const svgCountries = Array.from(svgMapDomEl.querySelectorAll("path"));

    if (!svgCountryDomEl || !svgCountries.length) {
      console.error("SVG country or countries not found!");
      return;
    }
    ...
    setMapTexture(
      globeSelectionOuterMesh.material,
      dataUris[hoveredCountryIdx]
    );
  };

  const updateMap = (uv = { x: 0, y: 0 }) => {
    const svgMapDomEl = svgMapRef.current;
    const svgCountries = Array.from(svgMapDomEl.querySelectorAll("path"));
    const countryNameEl = document.querySelector(".globe-info span.globe-span");
  ...
  return (
    <>
      <h2 className="globe-style-title albert-sans text-center text-uppercase  mt-lg-5 pt-md-5 mt-sm-0 ">
        Our Global Presence
      </h2>
      <div className="page-containers">
        <div className="globe-container" ref={containerRef}>
          ...
          <div className="globe-info">
            <span className="globe-span"></span>
          </div>
        </div>
        <svg id="map" ref={svgMapRef} className="map-color-style">
          <path
          ...
          />
          <path
